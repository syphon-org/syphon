...
import 'package:syphon/domain/settings/notification-settings/remote/actions.dart';
...
ThunkAction<AppState> initDeepLinks() => (Store<AppState> store) async {
      try {
        ...
        _deeplinkSubscription = uriLinkStream.listen((Uri? uri) {
          ...
          console.error('[streamUniLinks] $err');
...
Device generateDeviceId({String salt = ''}) {
  ...
  try {
    ...
    console.error('[generateDeviceId] $error');
    return Device(
      deviceId: defaultId,
      displayName: Values.appDisplayName,
    );
...
ThunkAction<AppState> checkUsernameAvailability() {
  return (Store<AppState> store) async {
    try {
      ...
      console.error('[checkUsernameAvailability] $error');
    ...
    try {
      ...
      if (auths['flows'] != null && auths['flows'].length > 0) {
        ...
        if (currentStage.length > 0) {
          console.info('[SetCredential] $currentStage');
      ...
      console.error('[setInteractiveAuth] $error');
    }
  };
}
...
ThunkAction<AppState> fetchHomeserver({String? hostname}) {
  return (Store<AppState> store) async {
    ...
    try {
      ...
      console.error('[selectHomserver] $error');
    }
  ...
  return (Store<AppState> store) async {
    try {
      ...
      store.dispatch(addAlert(
        ...
        message:
            DEBUG_MODE ? error.toString() : 'Failure to remove screen lock. Try again or contact support.',
        error: error,
      ));
      return false;
    }
