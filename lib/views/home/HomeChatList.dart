...
class HomeChatList extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final chats = useSelectorUnsafe<AppState, List<Chat>>(selectHomeChats) ?? [];
    final syncing = useSelectorUnsafe<AppState, bool>(selectSyncingStatus) ?? false;

    final currentUser = useSelectorUnsafe<AppState, User>((state) => state.authStore.user);
    final roomTypeBadgesEnabled = useSelectorUnsafe<AppState, bool>(
          (state) => store.state.settingsStore.roomTypeBadgesEnabled,
        ) ??
        true;

    final messagesAll = useSelectorUnsafe<AppState, Map<String, List<Message>>>(
          (state) => state.eventStore.messages,
        ) ??
        {};
    final decryptedAll = useSelectorUnsafe<AppState, Map<String, List<Message>>>(
          (state) => state.eventStore.messagesDecrypted,
        ) ??
        {};
    final searchMessages = useSelectorUnsafe<AppState, List<Message>>(
